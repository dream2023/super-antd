...
describe('schemaApiParser', () => {
  describe('SchemaApiToComponent', () => {
    test('正常请求，应先 loading，后显示组件', async () => {
      ...
      const warpper = render(<SuperProvider axios={axios}><SchemaApiToComponent api={api} /></SuperProvider>)
    ...
    test('无数据时，应返回为空', () => {
      const warpper = render(<SuperProvider axios={axios}><SchemaApiToComponent /></SuperProvider>)
