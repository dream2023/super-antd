...
import { SuperForm, SuperInput, withFormItem, WithFormItemConfigType, WithFormItemProps } from 'super-antd'

const Demo: FC<any> = (props) => {
  return <div data-testid="data">{JSON.stringify(props)}</div>
}

const App: FC<WithFormItemConfigType & WithFormItemProps> = ({ placeholderPrefix, needData, form = {}, ...resetProps }) => {
  const DemoWithFormItem = withFormItem(Demo, { placeholderPrefix, needData })
  return <SuperForm {...form} initialValues={{ a: '1' }} isResponsive={false}>
    <SuperInput name="a" label="a"></SuperInput>
    <ProFormDependency name={['a']}>
      {
        (data, form) => (
          <DemoWithFormItem data={data} form={form} {...resetProps} />
        )
      }
    </ProFormDependency>
  </SuperForm>
}
const setUp = (options: WithFormItemConfigType & WithFormItemProps, form?: any) => render(<App form={form} {...options} />)
const toBe = (data: any) => {
...
describe('withFormItem', () => {
  ...
  describe('联动对值的影响', () => {
    test('clearValueAfterReadonly', async () => {
      setUp({ name: 'a', needData: true, clearValueAfterReadonly: true, readonlyOn: (data: any) => Number(data.a) >= 2 })
      await waitFor(() => {
    ...
    test('clearValueAfterDisabled', async () => {
      setUp({ name: 'a', needData: true, clearValueAfterDisabled: true, disabledOn: (data: any) => Number(data.a) >= 2 })
      await waitFor(() => {
        expect(screen.getByLabelText('a')).toHaveValue('1')
