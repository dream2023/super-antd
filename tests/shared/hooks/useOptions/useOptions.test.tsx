...
describe('useOptions', () => {
  describe('options 为数组', () => {
    ...
    test('options 为数组', () => {
      ...
      expect(result.current.options).toEqual([{ label: 'a', value: 'a' }, { label: 'b', value: 'b' }])
    })

    test('optionsProp 存在', () => {
      const { result } = setup({
        options: [{ name: 'a', id: 1 }, { name: 'b', id: 2 }],
        optionsProp: { labelKey: 'name', valueKey: 'id' }
      })
      expect(result.current.loading).toBe(false)
      expect(result.current.options).toEqual([{ label: 'a', value: 1 }, { label: 'b', value: 2 }])
    })
  })
  ...
  describe('options 为 API', () => {
    ...
    test('正常请求', async () => {
      ...
      render(<SuperProvider axios={axios}>
        <Demo api={api} />
      </SuperProvider>)
      ...
      await waitFor(() => {
        ...
        expect(screen.getByTestId('options').textContent).toBe(JSON.stringify([{label: 'a', value: 'a'}, {label: 'b', value: 'b'}]))
      }, { timeout: 1000 })
    })
    ...
    test('当依赖项发生时，重新请求', async () => {
      ...
      const App = () => {
        ...
        return <SuperProvider axios={axios}>
          <Demo api={api} data={data} />
          <button onClick={() => setData({ a: 'a'})} data-testid="setdata">更改 data</button>
        </SuperProvider>
      }
      render(<App />)
